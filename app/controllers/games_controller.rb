...
class GamesController < ApplicationController
  ...
  def score
    ...
    letters_unchanged = params[:letters].split("")
    @guess = params[:guess]
    ...
    if in_grid == false
      @message = "Sorry but #{@guess} can't be built out of #{letters_unchanged.join(" | ").upcase}"
      ...
      session['score'] = (session['score'] || 0) + (@guess.length * 2)
    end
  end
end
